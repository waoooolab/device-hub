...
def test_allocate_placement_replay_rejects_cross_tenant_context_conflict() -> None:
    ...
    client.post(
        ...
        json=_command_envelope(
            {
                "device_id": "gpu-node-replay-tenant",
                "capabilities": ["compute.comfyui.local"],
            }
        ),
        headers={"Authorization": f"Bearer {token}"},
    ...
    pair_req = client.post(
        "/v1/devices/pairing/request",
        json=_command_envelope({"device_id": "gpu-node-replay-tenant"}),
        headers={"Authorization": f"Bearer {token}"},
    )
    code = pair_req.json()["payload"]["code"]
    client.post(
    ...
    first = client.post(
        ...
        json=_command_envelope(
            {
                "run_id": "run-api-replay-tenant-1",
                "task_id": "task-api-replay-tenant-1",
                "execution_profile": {
                    "execution_mode": "compute",
                    "inference_target": "none",
                    "resource_class": "gpu",
    ...
    assert conflict_event["payload"]["decision"]["reason_code"] == "tenant_context_conflict"
    assert conflict_event["payload"]["decision"]["resource_snapshot"]["tenant_id"] == "t2"
    assert conflict_event["payload"]["decision"]["resource_snapshot"]["tenant_active_leases"] == 0
